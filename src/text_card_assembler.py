...
--------------
  ...
  static.  A licensed background track is mixed at 35 % volume with a 1 s
  fade-out at the end.

Duration  : DEFAULT_DURATION = 20 s (configurable, 18–22 s recommended)
...
# ── Timing ────────────────────────────────────────────────────────────────────
DEFAULT_DURATION = 20.0      # seconds — comfortable reading time for 80-120 words
FPS              = 30
ZOOM_START       = 1.00
...
def _pick_music() -> Optional[Path]:
    """
    Return a random licensed track from assets/music/, or None if none available.
...
def _compose_frame(
    ...
    # Measure a dummy draw surface for wrapping (overlay draw is fine)
    y = TEXT_Y_START
    for para in paragraphs:
        ...
        for line in lines:
            ...
            y += line_h
        y += para_gap   # extra breathing room between paragraphs
