...
soundhelix (default, no API key needed)
  ...
  Good for voiceover videos; too upbeat for text card format.

dark-ambient (recommended for text cards — no API key needed)
  Generates 3 silent 65-second tracks named "dark-ambient-N.mp3".
  Silence is better than upbeat SoundHelix for text card atmosphere.
  _pick_music() matches them via the "dark-ambient" filename prefix.
  Run once: python3 -m src.download_safe_music --source dark-ambient
  Upgrade to real tracks later: --source jamendo

jamendo (optional upgrade — broader selection)
...
Usage:
  ...
  python3 -m src.download_safe_music --source dark-ambient   # 3 silent dark-ambient placeholders
...
# ── Dark-ambient silence track catalogue ──────────────────────────────────────
# Silent placeholder tracks generated by FFmpeg (anullsrc → libmp3lame).
# Filenames start with "dark-ambient-" so text_card_assembler._pick_music()
# always selects them over upbeat SoundHelix tracks.
# Silence is better than upbeat electronic for text card atmosphere.
# Replace with real Jamendo tracks when JAMENDO_CLIENT_ID is available.
DARK_AMBIENT_SILENCE_TRACKS = [
    {"n": 1, "filename": "dark-ambient-1.mp3", "track_id": "dark-ambient-silence-1"},
    {"n": 2, "filename": "dark-ambient-2.mp3", "track_id": "dark-ambient-silence-2"},
    {"n": 3, "filename": "dark-ambient-3.mp3", "track_id": "dark-ambient-silence-3"},
]
DARK_AMBIENT_LICENSE     = "CC0 1.0 Universal (Public Domain)"
DARK_AMBIENT_LICENSE_URL = "https://creativecommons.org/publicdomain/zero/1.0/"
...
# ── Dark-ambient source (silence placeholders) ─────────────────────────────────

def _download_dark_ambient(count: int = 3) -> list[str]:
    """
    Generate silent dark-ambient placeholder MP3 tracks using FFmpeg.

    Produces tracks named "dark-ambient-N.mp3". The "dark-ambient" prefix
    satisfies the keyword filter in text_card_assembler._pick_music(), ensuring
    the text card pipeline never falls back to upbeat SoundHelix tracks.

    Silence is the correct placeholder behavior:
      • No Content-ID risk
      • No jarring music-topic mismatch
      • Easy to replace: run --source jamendo when JAMENDO_CLIENT_ID is set

    Already-registered tracks are skipped so re-running is safe.
    FFmpeg must be installed (it is required by the rest of the pipeline anyway).
    """
    import subprocess
    ...
    generated: list[str] = []
    for track in DARK_AMBIENT_SILENCE_TRACKS:
        ...
        dest = MUSIC_DIR / track["filename"]
        log.info(f"Generating silent dark-ambient track: {track['filename']}")
        try:
            r = subprocess.run(
                [
                    "ffmpeg", "-y",
            ...
            if r.returncode != 0:
                raise RuntimeError(f"FFmpeg exit {r.returncode}: {r.stderr[-200:]}")
        except Exception as exc:
            log.error(f"Silent track generation failed: {exc}")
            if dest.exists():
                dest.unlink()
            continue
        ...
        _register_track(
            filename=track["filename"],
            source_url="generated://ffmpeg-anullsrc",
            license_type=DARK_AMBIENT_LICENSE,
            license_url=DARK_AMBIENT_LICENSE_URL,
            artist="CipherPulse / FFmpeg generated",
            title=f"Dark Ambient Placeholder {track['n']} (Silent)",
            duration_s=65,
            track_id=track["track_id"],
            registry=registry,
        )
        _save_registry(registry)
        generated.append(track["filename"])
        log.info(f"  ✓ {track['filename']} registered")

    if generated:
        log.info(
            f"Generated {len(generated)} silent placeholder track(s). "
            "Upgrade to real dark ambient: "
...
def download_tracks(
    ...
    Args:
        source:  "soundhelix" (default), "freepd" (dark ambient), or "jamendo"
    ...
    elif source == "dark-ambient":
        return _download_dark_ambient(count)
    elif source == "jamendo":
        return _download_jamendo(query=query, count=count, client_id=api_key)
    else:
        raise ValueError(f"Unknown source {source!r}. Use 'soundhelix', 'dark-ambient', or 'jamendo'.")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--source", default="soundhelix",
        choices=["soundhelix", "dark-ambient", "jamendo"],
        help=(
            "Music source. "
            "'soundhelix' = 16 CC0 electronic tracks, no key needed (default). "
            "'dark-ambient' = 3 silent CC0 dark-ambient placeholders, no key needed. "
            "'jamendo' = real CC dark ambient, requires JAMENDO_CLIENT_ID in .env."
        ),
    )
    parser.add_argument(
    ...
    if args.list:
        ...
        if not files:
            ...
            elif args.source == "dark-ambient":
                print("  FFmpeg must be installed — check that `ffmpeg` is in your PATH")
            else:
